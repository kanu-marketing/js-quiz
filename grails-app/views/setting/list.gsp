			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="passedScore" title="${message(code: 'setting.passedScore.label', default: 'Passed Score')}" />
					
						<g:sortableColumn property="totalQuiz" title="${message(code: 'setting.totalQuiz.label', default: 'Total Quiz')}" />
				...
				<g:each in="${settingInstanceList}" status="i" var="settingInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${settingInstance.id}">${fieldValue(bean: settingInstance, field: "passedScore")}</g:link></td>
					
						<td>${fieldValue(bean: settingInstance, field: "totalQuiz")}</td>
